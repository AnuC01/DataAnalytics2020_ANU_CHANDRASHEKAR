...
set.seed(2)
train = sample(392,196)
lm.fit <- lm(mpg~horsepower, date = Auto, subset = train)
mean((mpg-predict(lm.fit,Auto))[-train]^2)
lm.fit2 <- lm(mpg~poly(horsepower,2), date = Auto, subset = train)
mean((mpg-predict(lm.fit,Auto))[-train]^2)
lm.fit3 <- lm(mpg~poly(horsepower,3), data = Auto, subset = train)
mean((mpg-predict(lm.fit3,Auto))[-train]^2)

#In-Class Work: Random Forest example
...
model_dt <- train(Condition~., date = TrainSet, model = "rpart")
...
model_dt_vs = predict(model_dt, newdate = ValidSet)
